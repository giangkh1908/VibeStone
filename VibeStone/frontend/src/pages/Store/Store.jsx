  ...
  useEffect(() => {
    ...
    if (searchTerm) {
      ...
      result = result.filter((product) => {
        ...
        const searchTermsNoAccent = searchTerms.map((term) =>
          removeAccents(term)
        );

        return searchTermsNoAccent.every(
          (term) => productName.includes(term) || productDesc.includes(term)
        );
      });
    }
